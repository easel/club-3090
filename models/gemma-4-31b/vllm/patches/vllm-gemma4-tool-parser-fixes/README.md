...
per-token-head KV cache page-size alignment) on the `docker-compose.gemma-mtp-int8.yml`
...
`docker-compose.gemma-mtp-int8.yml` — the per-token-head KV variant (INT8 default on
